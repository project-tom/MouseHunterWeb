...
<!----------------------------------회원탈퇴 테이블---------------------------------->
		<div id="main">
		<p align="center" style="font-size:30pt; padding-top: 100px" > 회원 정보를 확인해 주십시오.<p>
		<c:if test="${param.flag == 'Modify' }">
			<c:set var="uri" value="../Check.user?flag=${param.flag }"></c:set>
		</c:if>
		<c:if test="${param.flag == 'Withdrawal' }">
			<c:set var="uri" value="../Check.user?flag=${param.flag }"></c:set>
		</c:if>
		<div class="container">
			...
			<table class="table" >
				...
				<tr>
					...
					<td><input type="text" name="user_id" style="width:300px;"></td>
				</tr>	
				<tr>
			...
			<hr/>
			<button class="btn btn-default pull-right" type="submit" >확인</button>
